...
public class ComposeWarningsGuard extends WarningsGuard {
  ...
  private Iterable<WarningsGuard> descendingSet(TreeSet<WarningsGuard> ts) {
      List<WarningsGuard> result = new ArrayList<WarningsGuard>();
      for (WarningsGuard guard:ts)
        result.add(0, guard);
      return result;
  }
  ...
  void addGuard(WarningsGuard guard) {
    if (guard instanceof ComposeWarningsGuard) {
      ...
      addGuards(descendingSet( ((ComposeWarningsGuard) guard).guards));
    } else {
      numberOfAdds++;
  ...
  ComposeWarningsGuard makeEmergencyFailSafeGuard() {
    ...
    for (WarningsGuard guard : descendingSet(guards)) {
